...
docker-compose run --rm -u 33 cli core install --url=localhost:$HOST_PORT --title=Charitable --admin_user=admin --admin_password=password --admin_email=test@test.com >/dev/null
...
# Activate Charitable
echo -e $(status_message "Activating Charitable...")
docker-compose run --rm cli plugin activate charitable >/dev/null
